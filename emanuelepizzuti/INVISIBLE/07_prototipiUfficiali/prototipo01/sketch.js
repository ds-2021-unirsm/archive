...
// ___________
//
// BROKEN NEWS! (versione 01) by emanuelepizzuti [journalism, sentiment]
...
//-- resetSketch - dopo aver cliccato il pulsante reset...
...
//-- RiTa
// sostituisce una parola a caso nel testo ogni 2 secondi
function nextWord() {
  let words = RiTa.tokenize(titolo);
  ...
  for (let i = r; i < words.length + r; i++) {
    ...
    words[idx] = next; // sostituisce la parola

    break;
  }
  //--fine > parola sostituita!

  // ri-unisco tutte le parole dell'array in una nuova stringa
  titolo = RiTa.untokenize(words);

  // eseguo la sentiment analysis ogni 4 parole cambiate
  counter++;
  
    sentimentAnalysis(titolo);

  timeout = setTimeout(nextWord, 2000);
}
...
function response2(sentResp2) {
  ...
  console.log("sentimento trovato: " + sentimentType);
  scegliColore();
}
...
function esegui() {
  ...
  textStyle(BOLD);
  text(
    "Il titolo cambia finché non diventa neutrale",
    windowWidth / 2,
    windowHeight / 2 - 338
  );
