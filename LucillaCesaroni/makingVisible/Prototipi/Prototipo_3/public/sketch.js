...
// -
//
// Metamorfosi 0.3 by Lucilla Cesaroni [comunicare, parole, icone, emozioni, inclusione]
...
let wImg = 50; // Larghezza immagine Noun Project
let hImg = 50; // Altezza immagine Noun Project
...
window.onload = function() {
  ...
  var f1 = gui.addFolder("Scegli la tipologia delle icone");
  f1.add(parametri, "tipologiaIcone", 0, 29);
};
...
function visualizzaRisposta(risposta) {
  ...
  numeroArray = int(parametri.tipologiaIcone);

  console.log("numeroArray" + numeroArray)
  // Carico l'immagine
  fetch("/" + string + numeroArray)
    .then(data => data.json())
    .then(json => {
      console.log(json);
      loadImage(json, salvaimmagine);
    });
...
function windowResized() {
  ...
  if (w >= h) {
    ...
    video.size((h * 1.4) / scala, h / scala);
  } else {
    // Smartphone
    // console.log("caso2");
    let scala = 1.3;
  ...
  speakbutton.position(
    origineX,
    h -
    ((origineY + video.height + video.height / 60 + video.height / 4) * 0.45) /
    5
  );
  pausebutton.position(
    w / 2 -
